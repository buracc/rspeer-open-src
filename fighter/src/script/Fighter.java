...
public class Fighter extends Script {
    ...
    private static final String NPC_NAME = "Zombie";
    private static final Predicate<Npc> NPC_PRED = x -> x.getName().equals(NPC_NAME) && (x.getTargetIndex() == Players.getLocal().getIndex() || x.getTargetIndex() == -1);

    @Override
    public int loop() {
        Item food = Inventory.getFirst(FOOD);
        Player local = Players.getLocal();
        Npc targetNpc = Npcs.getNearest(NPC_PRED);
